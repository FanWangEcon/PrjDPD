...
# 2.C Results/data output main folder
spt_results <- "PIRE/team/xiuqi_yang/PM2.5_tables/final_21group_output"
spt_path_out_main <- file.path(spt_root, spt_results,
                          fsep = .Platform$file.sep)
...
for (it_demo_set in c(1,2)) {
  ...
  if (it_demo_set == 1) {
    ...
    spt_path_out <- file.path(spt_path_out_main, "ineq_demo_edu_groupsm2_bl_thres", fsep = .Platform$file.sep)

  } else if (it_demo_set == 2) {
    stv_grp_demo <- "mgrtgroupsm2_bl"
    arv_label_demo <- c("mgrtgroupsm2_bl", "mgrtgroupsm2")

    # 2.C Results/data output folder
    spt_path_out <- file.path(spt_path_out_main, "ineq_demo_mgrtgroupsm2_bl_thres", fsep = .Platform$file.sep)
  ...
  for (fl_temp_bound in ar_fl_temp_bound) {
    ...
    print(df_excburden_percentiles_keys)
      ...
      if (it_file_ctr == 1) {
        df_thres_jnt_main <- df_excburden_percentiles_keys
      } else {
        df_thres_jnt_main <- bind_rows(
          df_thres_jnt_main, df_excburden_percentiles_keys)
      }        

  }

  # df_thres_jnt_main_sel <- df_thres_jnt_main %>%
  #   select(one_of(    
  #     str_prefix_demo,
  #     stv_grp_demo, arv_label_demo, 
  #     stv_grp_loc, arv_label_loc
  #   ), within_group_percentile, contains("pm10_p"))

  # df_thres_jnt_main_long <- df_thres_jnt_main_sel %>%
  #   pivot_longer(cols = starts_with('pm10_p'),
  #                names_to = c('within_person_percentile'),
  #                names_pattern = paste0("pm10_p(.*)"),
  #                values_to = "value") %>%
  #   mutate(within_group_percentile = within_group_percentile*100)
  ...
  readr::write_csv(df_thres_jnt_main, spn_output_file, na="0")
  if (verbose_debug) {
    print(glue::glue(
